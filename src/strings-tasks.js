...
function removeFirstOccurrences(str, value) {
  ...
  if (index !== -1) {
    ...
    const b = str.slice(index + value.length);
    return a + b;
  }
  return str;
}
...
function isPalindrome(str) {
  let rev = str.toLowerCase().split('');
  rev = rev.filter((i) => i !== '!' && i !== '?' && i !== ' ' && i !== ',');
  const first = rev.join('');
  if (first === rev.reverse().join('')) {
    return true;
  }
  return false;
}
...
function reverseWords(str) {
  ...
  const answ = [];
  for (let i = 0; i < arr.length; i += 1) {
    answ.push(arr[i].split('').reverse().join(''));
  }
  return answ.join(' ');
}
...
function invertCase(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    if (arr[i] === arr[i].toUpperCase()) {
      answ.push(arr[i].toLowerCase());
    } else {
      answ.push(arr[i].toUpperCase());
    }
  }
  return answ.join('');
}
